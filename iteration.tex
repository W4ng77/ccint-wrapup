...
it counts --- moved literal-mention recall from $69.0\%$ to $99.3\%$ at a cost
of raising false positives from $0.5\%$ to $1.0\%$ (Figure~\ref{fig:prompt}).

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth]{iter_prompt.pdf}
\caption{Entity-extraction recall on three probe sets before and after the
prompt revision. The middle set is the diagnostic one: these are posts that
literally contain the word \emph{Canada}, so any answer below $100\%$ is a
failure of the instrument rather than a hard judgement call.}
\label{fig:prompt}
\end{figure}

What remains after the fix is more interesting than what it removed. Of the
posts the corrected extractor still misses, \textbf{none} contains the word
\emph{Canada} in any form. They read \emph{``New post from \#Qilin: Coldfish
Seafood''} --- a real Canadian victim, named, with no country marker anywhere
in the text. No amount of prompting will let a 4B model know that Coldfish
Seafood is Canadian.
...
through the cyber judgement before counting. This is an oracle: it asks what
the platform would have yielded had we known in advance exactly what to look
for.
...
650 to 906, an increase of $39\%$, with $65\%$ of the additions attributable
to the Canadian side of the conjunction.
...
presence at all, because establishing that requires the per-event oracle search
run here, and that search is only cheap if the event count is small. This
...
Oracle collection mode & A measurement-only collection path still writes to
...
retrieved on 22 September 2026, and an oracle search executed the same day.
