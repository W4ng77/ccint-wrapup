...
All figures are generated by \code{ccint viz} directly from the database; none
are hand-drawn, and each carries the parameters it was produced under.

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/fig_raw_vs_organic.pdf}
\caption{\textbf{The project's signature chart.} Plotting raw volume alone would
show \code{ransomware} as the second-largest topic. Removing 11 broadcast
accounts takes it from 105 posts to 17, contributed by 12 authors. Any chart of
this corpus that shows only raw counts is misleading by construction.}
\label{fig:rawvsorganic}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/fig_topic_trajectory.pdf}
\caption{\textbf{Topic trajectories.} The gap between the blue and orange lines
is feed-driven volume. In \code{ransomware} the orange line is flat near zero
throughout: there is essentially no organic discussion under that label at all.
In \code{fraud\_financial} the three lines are almost coincident --- that is what
distributed discussion looks like here.}
\label{fig:trajectory}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/fig_topic_week_heatmap.pdf}
\caption{\textbf{Topic composition by week.} Normalised to each week's share so
that collection-volume drift does not masquerade as attention shifting. Raw
counts are printed in each cell precisely because the shares are unreliable at
this size --- a 0.6 share of a 20-post week is 12 posts.}
\label{fig:heatmap}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/fig_actor_topic_network.pdf}
\caption{\textbf{Actor--topic network.} \code{other}, \code{data\_breach} and
\code{fraud\_financial} are ringed by large clouds of small organic accounts.
\code{ransomware} is instead a small core of large red nodes. This is the same
finding as Figure~\ref{fig:rawvsorganic}, in the form that makes the
\emph{structure} of participation visible rather than just its magnitude.}
\label{fig:actortopic}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.72\textwidth]{figures/fig_concentration.pdf}
\caption{\textbf{Posting concentration.} Gini falls from 0.38 to 0.23 when
broadcast accounts are excluded. The area between the curves is exactly what the
filter removes.}
\label{fig:concentration}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/fig_source_uptake.pdf}
\caption{\textbf{External source uptake.} Which sources many accounts cite
\emph{independently} (\code{cbc.ca}, \code{krebsonsecurity.com}, \code{reuters.com})
versus which are one feed reposting its own domain. \textbf{This is deliberately
not drawn as a network.} An account--account co-sharing graph was built first and
rejected: 115 of the 121 participating accounts appear in exactly one shared
source, giving a transitivity of 0.907 --- the graph is a set of disjoint
cliques with no topology to read. Drawing it as a node-link diagram would imply
structure that is not present. A genuine propagation graph requires
\code{app.bsky.feed.getRepostedBy}, which this project does not yet collect.}
\label{fig:sourceuptake}
\end{figure}
